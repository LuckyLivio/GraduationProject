...
打开 **http://127.0.0.1:8765/web/**。仓库包含约 17KB 的首轮已训练模型，无需先训练即可体验。

- **实时推理：**选择方法、场景与阻尼倍率，开始运行；点击地图改变目标。每一步调用真实模型与规划器。
...
**当前实现是混合模型：**3 个小型 MLP 从交互转移学习“位置 → 阻尼”，再结合已知动作积分与障碍反弹规则预测未来。训练未读取真实阻尼场标签，推理不查询真实阻尼场。当前模型不读取历史，局部物理辨识基线读取最近转移，因此在线适应能力不同，实验解释中明确保留这一差异。历史条件神经模型尚未实现。
...
主系统方向继续，算法机制根据证据调整。首先分析集成分歧为什么不能识别隐藏的整体阻尼变化，再评估近期预测残差或历史条件模型；每次只改变一个主要机制，保留强物理辨识基线。原有 `research-plan.md` 等文档描述完整计划，实际已实现范围和差异以本 README、工作记录与实验协议为准。
