...
**最新已试跑（2026-09-17）：**对象交互世界模型已完成 24,000 条接触转移、9 个模型训练、40 个开发场景预测与 32 个方法规划回合。[打开推物试验台](http://127.0.0.1:8765/web/pushing.html)：同一箱子的四种推法、四个未来，以及预测/真实执行对照。历史模型相对同容量无历史模型一步位置误差降低约 20.6%，十步没有改善；物理辨识仍更准，尚未验证主动试探创新。见[真实结果与复现](docs/pushing-pilot-findings.md)及[选题论文依据](docs/world-model-upgrade-decision.md)。下面保留原导航系统的入口与实验历史。
...
19. [推物首轮协议与补充容量对照](docs/pushing-pilot-protocol.md)
20. [推物模型实际训练、结果与复现](docs/pushing-pilot-findings.md)

## 下一步

对象接触首轮已完成，自训练预测与规划闭环可用，长期推演误差尚大。下一项建议是多步训练与精确位姿/停稳任务；主动试探和视觉输入待后续验证。早期 `research-plan.md` 等文档保留当时计划，实际已实现范围以本 README、工作记录与各批实验报告为准。
